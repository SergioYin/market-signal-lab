...
- [Static Visual Release Comparison](reports/static-visual-release-comparison.md) - compare the v1.30.7 static visual receipt baseline with the current static receipt scan and reviewer checklist.
...
For the static visual release comparison, run `python -m market_signal_lab.cli --static-visual-release-comparison`.
...
Generate a deterministic public-review matrix for deciding whether a static backtest
artifact is safe to release and promote:

```bash
python -m market_signal_lab.cli --reviewer-decision-matrix
```

This writes `reports/reviewer-decision-matrix.md` and
`reports/reviewer-decision-matrix.json` with Release Gate and Promotion Gate labels.
...
Open `reports/public-demo-evidence-receipt.md` for the focused public demo receipt covering gallery/backtest artifact hashes, fixture provenance paths, and explicit no-live-data/no-advice claims. Regenerate it with:
...
Open `reports/visual-acceptance-bundle.md` for the bounded visual acceptance bundle tying the static walkthrough, gallery, visual receipt, acceptance receipt index, reviewer acceptance scorecard, cold-user route, artifact hashes, and no-live-data/no-advice boundaries together. Regenerate it with:

```bash
python -m market_signal_lab.cli --visual-acceptance-bundle
```

Open `reports/static-visual-capture-checklist.md` for the public-safe checklist that tells cold reviewers how to capture a local static gallery screenshot or GIF while preserving no-live-data, no-broker/account, no-order, no-position-sizing, no-forecast, no-recommendation, and no-advice boundaries. Regenerate it with:

```bash
python -m market_signal_lab.cli --static-visual-capture-checklist
```

Open `reports/static-visual-release-comparison.md` for the release-to-release static visual receipt comparison that checks the v1.30.7 receipt baseline against the current static receipt scan, source receipt artifacts, hashes, and reviewer checklist. Regenerate it with:
...
- [v1.30.8 Release Notes](docs/release-v1.30.8.md)
